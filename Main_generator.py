...
injection_parameters = dict(
    ...
    geocent_time=1126259642.413, ra=1.375, dec=-1.2108, t_echo=1)
...
def lal_binary_black_hole_echo(
    ...
    dt = t_echo
    signal['plus'] += signal['plus'] * np.exp(-1j * 2 * np.pi * dt * frequency_array)
    signal['cross'] += signal['cross'] * np.exp(-1j * 2 * np.pi * dt * frequency_array)
    return signal
...
for key in ['a_1', 'a_2', 'tilt_1', 'tilt_2', 'phi_12', 'phi_jl', 'psi', 'ra',
            'dec', 'geocent_time', 'phase', 'mass_1', 'mass_2', 'iota']:
